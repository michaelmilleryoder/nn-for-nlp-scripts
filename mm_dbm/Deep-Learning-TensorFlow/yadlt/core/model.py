...
class Model(object):
    ...
    def compute_regularization(self, vars):
        ...
        if self.regtype != 'none':
            ...
            return tf.multiply(self.l2reg, regularizers)
        else:
            return None
    ...
    def _create_cost_function_node(self, model_output, ref_input,
        ...
        with tf.name_scope("cost"):
            if self.loss_func == 'cross_entropy':
                ...
                cost = - tf.reduce_mean(
                    tf.add(
                        tf.multiply(ref_input, tf.log(clip_inf)),
                        tf.multiply(tf.subtract(1.0, ref_input), tf.log(clip_sup))
                    ))
                ...
                cost = tf.sqrt(tf.reduce_mean(
                    tf.square(tf.subtract(ref_input, model_output))))
